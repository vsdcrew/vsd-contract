...
contract MockUniswapV2PairLiquidity is IUniswapV2Pair {
    ...
    function setReserve(uint112 newReserve0, uint112 newReserve1) external {
        reserve0 = newReserve0;
        reserve1 = newReserve1;
    }

    function setToken(address newToken0, address newToken1) external {
        _token0 = newToken0 < newToken1 ? newToken0 : newToken1;
        _token1 = newToken0 < newToken1 ? newToken1 : newToken0;
    }

    function setTokenReserve(address token, uint112 reserve) external {
        if (token == _token0) {
            reserve0 = reserve;
        } else {
            require (token == _token1, "token not in the pair");
            reserve1 = reserve;
        }
    }
